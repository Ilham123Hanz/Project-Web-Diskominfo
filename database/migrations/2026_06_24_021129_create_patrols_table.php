<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {
    public function up(): void {
        Schema::create('patrols', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained()->onDelete('cascade'); 
            $table->string('attendance_info'); // Keterangan waktu presensi terkait
            $table->enum('main_menu', ['Bug Hunter', 'CTI', 'Laporan Insiden', 'Patroli Siber', 'Sosial Media', 'Vul/Pen Test']);
            $table->string('category');
            $table->string('agency_name');
            $table->string('target_url')->nullable();
            $table->enum('threat_level', ['Low', 'Medium', 'High', 'Critical']);
            $table->text('description');
            $table->text('coordination_note')->nullable();
            $table->string('file_evidence')->nullable(); // Mendukung PDF, DOCX, XLSX
            $table->enum('status', ['Pending', 'Perlu Perbaikan', 'Verified'])->default('Pending');
            $table->text('admin_correction')->nullable(); 
            $table->timestamps();
        });
    }

    public function down(): void {
        Schema::dropIfExists('patrols');
    }
};