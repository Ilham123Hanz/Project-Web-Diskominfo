...
<head>
    ...
    <title>Gate checkpoint Absensi Admin - SIP-O-SIBER</title>
...
<body>
    ...
    <div class="container main-container">
        ...
        <div class="row g-2 mb-3">
            <div class="col-md-6">
                <div class="p-2 bg-white rounded shadow-sm d-flex justify-content-between align-items-center">
                    <span class="text-muted small fw-bold"><i class="fas fa-user-shield text-primary me-1"></i> Level Otoritas:</span>
                    <span class="badge bg-primary telemetry-badge">Administrator</span>
                </div>
            </div>
            <div class="col-md-6">
                <div class="p-2 bg-white rounded shadow-sm d-flex justify-content-between align-items-center">
        ...
        <div class="card card-custom">
            <h4 class="fw-bold text-dark mb-1">Modul Absensi Harian Reguler Admin</h4>
            <p class="text-muted small mb-4">Pencatatan log presensi mandiri khusus internal user Administrator (Batas Kerja 07:30 - 16:00 WIB).</p>
            ...
            <form action="{{ route('admin.attendance.store') }}" method="POST" id="attendanceForm">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label form-label-custom">Nama Administrator</label>
                        <input type="text" class="form-control form-control-custom" value="{{ Auth::user()->name }}" readonly>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label form-label-custom">Aksi Validasi Log</label>
                    ...
                    <div class="col-12">
                        <label class="form-label form-label-custom">Catatan / Keterangan Masuk & Keluar</label>
                        <textarea name="notes" class="form-control form-control-custom" rows="4" 
                                  placeholder="{{ !$hasMasuk ? 'Tuliskan catatan kondisi sistem siber server saat Anda mulai berjaga...' : 'Tuliskan catatan status akhir server sebelum Anda pulang...' }}"></textarea>
                    </div>

                    <div class="col-12 text-end border-top pt-3 mt-4">
                        @if(!$hasMasuk)
                            <button type="submit" id="btn-submit-attendance" class="btn btn-attendance">
                                <i class="fas fa-check-square me-2"></i> Kirim Kehadiran Masuk Admin
                            </button>
                        @else
                            <button type="submit" id="btn-submit-attendance" class="btn btn-attendance clock-out">
                                <i class="fas fa-sign-out-alt me-2"></i> Kirim Kehadiran Pulang Admin
                            </button>
                        @endif
                    </div>
                </div>
